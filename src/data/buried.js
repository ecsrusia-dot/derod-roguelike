...
export const BURIED_GHOSTS = [
  ...
  // ═══ 1.157.0 — 전 적 커버 확장 (PM: 500층 넘게 올랐는데 괴이가 아니라 못 잡는다) ═══
  // 82종 적 전부가 괴이가 된다 — 어느 층 어느 방에서든 HP 25% 이하 + 등급 제령부면 제령 시도 가능.
  // 등급 관례 유지: 저층 일반=적령 / 중층 일반·저층 강적=녹령 / 심층=청령 / 최심층 강적=남령.
  // ── 적령 +16 (저층 일반) ──
  GH({ id: 'gh_robber',   enemyKey: 'graverobber',    rank: 'red', name: '도굴꾼의 원혼',  passive: { goldPct: 6 },        active: { apply: [{ s: 'shatter', n: 1 }] }, aDesc: '적 [파쇄] 1' }),
  GH({ id: 'gh_chound',   enemyKey: 'cryptHound',     rank: 'red', name: '납골당 들개',    passive: { crit: 3 },           active: { power: { stat: 'dex', pct: 15 } }, aDesc: '기교 15% 타격' }),
  GH({ id: 'gh_rspider',  enemyKey: 'rotSpider',      rank: 'red', name: '부패 거미줄',    passive: { statusChance: 5 },   active: { apply: [{ s: 'poison', n: 1 }] }, aDesc: '적 [중독] 1' }),
  GH({ id: 'gh_tslime',   enemyKey: 'tombSlime',      rank: 'red', name: '무덤 점액',      passive: { hpPct: 4 },          active: { self: [{ s: 'regen', n: 1 }] },   aDesc: '자신 [재생] 1' }),
  GH({ id: 'gh_asoldier', enemyKey: 'ancientSoldier', rank: 'red', name: '고대병의 충혼',  passive: { physPct: 4 },        active: { power: { stat: 'str', pct: 15 } }, aDesc: '완력 15% 타격' }),
  GH({ id: 'gh_bwalker',  enemyKey: 'boneWalker',     rank: 'red', name: '백골 행자',      passive: { takenPct: -2 },      active: { self: [{ s: 'guard', n: 1 }] },   aDesc: '자신 [수호] 1' }),
  GH({ id: 'gh_mandra',   enemyKey: 'mandragora',     rank: 'red', name: '비명 뿌리',      passive: { healPct: 6 },        active: { apply: [{ s: 'confuse', n: 1 }] }, aDesc: '적 [혼란] 1' }),
  GH({ id: 'gh_prats',    enemyKey: 'plagueRats',     rank: 'red', name: '역병 쥐혼',      passive: { statusChance: 5 },   active: { apply: [{ s: 'poison', n: 1 }] }, aDesc: '적 [중독] 1' }),
  GH({ id: 'gh_hornet',   enemyKey: 'hellHornet',     rank: 'red', name: '지옥벌 침',      passive: { crit: 3 },           active: { apply: [{ s: 'bleed', n: 1 }] },  aDesc: '적 [출혈] 1' }),
  GH({ id: 'gh_bogHusk',  enemyKey: 'bogHusk',        rank: 'red', name: '늪 껍데기 혼',   passive: { dodge: 3 },          active: { apply: [{ s: 'bind', n: 1 }] },   aDesc: '적 [속박] 1' }),
  GH({ id: 'gh_thorn',    enemyKey: 'thornCreeper',   rank: 'red', name: '가시덤불 정령',  passive: { barrierPct: 4 },     active: { apply: [{ s: 'bleed', n: 1 }] },  aDesc: '적 [출혈] 1' }),
  GH({ id: 'gh_rscout',   enemyKey: 'riftScout',      rank: 'red', name: '균열 첨병 혼',   passive: { expPct: 5 },         active: { power: { stat: 'dex', pct: 15 } }, aDesc: '기교 15% 타격' }),
  GH({ id: 'gh_gimp',     enemyKey: 'gloomImp',       rank: 'red', name: '어스름 꼬마귀',  passive: { magPct: 4 },         active: { power: { stat: 'int', pct: 15 } }, aDesc: '지혜 15% 타격' }),
  GH({ id: 'gh_sMass',    enemyKey: 'slimeMass',      rank: 'red', name: '고름 덩이혼',    passive: { hpPct: 4 },          active: { healPct: 4 },                      aDesc: 'HP 4% 회복' }),
  GH({ id: 'gh_bandit',   enemyKey: 'graveBandit',    rank: 'red', name: '야도의 원귀',    passive: { goldPct: 6 },        active: { power: { stat: 'dex', pct: 15 } }, aDesc: '기교 15% 타격' }),
  GH({ id: 'gh_sservant', enemyKey: 'stoneServant',   rank: 'red', name: '석조 시종혼',    passive: { takenPct: -2 },      active: { barrier: 8 },                      aDesc: '보호막 +8' }),
  // ── 녹령 +17 (중층 일반 · 저층 강적) ──
  GH({ id: 'gh_traider',  enemyKey: 'tombRaider',     rank: 'green', name: '약탈자의 집념', passive: { goldPct: 10 },      active: { power: { stat: 'str', pct: 25 } }, aDesc: '완력 25% 타격' }),
  GH({ id: 'gh_franger',  enemyKey: 'fallenRanger',   rank: 'green', name: '타락 사수 혼',  passive: { crit: 5 },          active: { power: { stat: 'dex', pct: 25 } }, aDesc: '기교 25% 타격' }),
  GH({ id: 'gh_fhound',   enemyKey: 'frostHound',     rank: 'green', name: '서리 이빨',     passive: { physPct: 7 },       active: { apply: [{ s: 'weaken', n: 2 }] }, aDesc: '적 [약화] 2' }),
  GH({ id: 'gh_gargoyle', enemyKey: 'gargoyleWatch',  rank: 'green', name: '가고일 눈동자', passive: { takenPct: -4 },     active: { self: [{ s: 'guard', n: 2 }] },   aDesc: '자신 [수호] 2' }),
  GH({ id: 'gh_fshaman',  enemyKey: 'frostShaman',    rank: 'green', name: '언 뼈 주술혼',  passive: { magPct: 7 },        active: { apply: [{ s: 'weaken', n: 2 }] }, aDesc: '적 [약화] 2' }),
  GH({ id: 'gh_cimp',     enemyKey: 'crevasseImp',    rank: 'green', name: '틈새 요괴',     passive: { dodge: 5 },         active: { power: { stat: 'int', pct: 25 } }, aDesc: '지혜 25% 타격' }),
  GH({ id: 'gh_mleopard', enemyKey: 'mireLeopard',    rank: 'green', name: '수렁 표범혼',   passive: { crit: 5 },          active: { apply: [{ s: 'bleed', n: 2 }] },  aDesc: '적 [출혈] 2' }),
  GH({ id: 'gh_wyvern',   enemyKey: 'wyvernFledgling',rank: 'green', name: '어린 용의 혼',  passive: { physPct: 7 },       active: { power: { stat: 'str', pct: 25 } }, aDesc: '완력 25% 타격' }),
  GH({ id: 'gh_gcrab',    enemyKey: 'giantCrab',      rank: 'green', name: '집게 원혼',     passive: { takenPct: -4 },     active: { barrier: 15 },                     aDesc: '보호막 +15' }),
  GH({ id: 'gh_viper',    enemyKey: 'viperNest',      rank: 'green', name: '살무사 무리혼', passive: { statusChance: 8 },  active: { apply: [{ s: 'poison', n: 2 }] }, aDesc: '적 [중독] 2' }),
  GH({ id: 'gh_sludge',   enemyKey: 'sludgeMass',     rank: 'green', name: '오니 진창',     passive: { hpPct: 6 },         active: { apply: [{ s: 'bind', n: 1 }] },   aDesc: '적 [속박] 1' }),
  GH({ id: 'gh_rune',     enemyKey: 'runeSentinel',   rank: 'green', name: '룬 파수혼',     passive: { startSpPct: 12 },   active: { power: { stat: 'int', pct: 25 } }, aDesc: '지혜 25% 타격' }),
  GH({ id: 'gh_btooth',   enemyKey: 'bladetooth',     rank: 'green', name: '칼이빨 혼',     passive: { crit: 5 },          active: { power: { stat: 'dex', pct: 25 } }, aDesc: '기교 25% 타격' }),
  GH({ id: 'gh_mimic',    enemyKey: 'graveMimic',     rank: 'green', name: '미믹의 탐욕',   passive: { dropLuck: 1 },      active: { power: { stat: 'str', pct: 25 } }, aDesc: '완력 25% 타격' }),
  GH({ id: 'gh_bgiant',   enemyKey: 'boneGiant',      rank: 'green', name: '거인 백골혼',   passive: { hpPct: 6 },         active: { power: { stat: 'str', pct: 28 } }, aDesc: '완력 28% 타격' }),
  GH({ id: 'gh_ftiger',   enemyKey: 'fenTiger',       rank: 'green', name: '늪호랑이 혼',   passive: { physPct: 7 },       active: { apply: [{ s: 'bleed', n: 2 }] },  aDesc: '적 [출혈] 2' }),
  GH({ id: 'gh_wrath',    enemyKey: 'wrathSpawn',     rank: 'green', name: '분노의 잔재',   passive: { physPct: 7 },       active: { self: [{ s: 'rage', n: 1 }] },    aDesc: '자신 [격노] 1' }),
  // ── 청령 +17 (심층 일반 · 중층 강적) ──
  GH({ id: 'gh_paleelf',  enemyKey: 'paleElf',        rank: 'cyan', name: '창백한 사수혼',  passive: { crit: 7 },          active: { power: { stat: 'dex', pct: 32 } }, aDesc: '기교 32% 타격' }),
  GH({ id: 'gh_hourglass',enemyKey: 'hourglassKeeper',rank: 'cyan', name: '모래시계 수호령', passive: { startSpPct: 15 },  active: { self: [{ s: 'evade', n: 1 }, { s: 'guard', n: 2 }] }, aDesc: '자신 [수호] 2 + [잔영] 1' }),
  GH({ id: 'gh_dryad',    enemyKey: 'rotDryad',       rank: 'cyan', name: '부패 드라이어드', passive: { healPct: 12 },     active: { healPct: 8 },                      aDesc: 'HP 8% 회복' }),
  GH({ id: 'gh_apostle',  enemyKey: 'abyssApostle',   rank: 'cyan', name: '나락 사도혼',    passive: { magPct: 9 },        active: { apply: [{ s: 'curse', n: 1 }] },  aDesc: '적 [저주] 1' }),
  GH({ id: 'gh_fseer',    enemyKey: 'frostSeer',      rank: 'cyan', name: '서리 예언령',    passive: { magPct: 9 },        active: { apply: [{ s: 'weaken', n: 3 }] }, aDesc: '적 [약화] 3' }),
  GH({ id: 'gh_lstatue',  enemyKey: 'livingStatue',   rank: 'cyan', name: '석상의 진혼',    passive: { takenPct: -5 },     active: { barrier: 25 },                     aDesc: '보호막 +25' }),
  GH({ id: 'gh_janus',    enemyKey: 'janusWarden',    rank: 'cyan', name: '야누스 이면혼',  passive: { dodge: 7 },         active: { self: [{ s: 'evade', n: 2 }] },   aDesc: '자신 [잔영] 2' }),
  GH({ id: 'gh_bishop',   enemyKey: 'ancientBishop',  rank: 'cyan', name: '고대 주교령',    passive: { healPct: 12 },      active: { self: [{ s: 'regen', n: 2 }] },   aDesc: '자신 [재생] 2' }),
  GH({ id: 'gh_dpriest',  enemyKey: 'deepSeaPriest',  rank: 'cyan', name: '심해 사제령',    passive: { barrierPct: 8 },    active: { barrier: 22 },                     aDesc: '보호막 +22' }),
  GH({ id: 'gh_ninja',    enemyKey: 'masterNinja',    rank: 'cyan', name: '무명 닌자혼',    passive: { crit: 7 },          active: { power: { stat: 'dex', pct: 30 }, apply: [{ s: 'bleed', n: 2 }] }, aDesc: '기교 30% + [출혈] 2' }),
  GH({ id: 'gh_vbrute',   enemyKey: 'voidBrute',      rank: 'cyan', name: '공허 만행혼',    passive: { physPct: 9 },       active: { power: { stat: 'str', pct: 32 } }, aDesc: '완력 32% 타격' }),
  GH({ id: 'gh_vault',    enemyKey: 'vaultKeeper',    rank: 'cyan', name: '금고지기 원혼',  passive: { goldPct: 15 },      active: { apply: [{ s: 'shatter', n: 2 }] }, aDesc: '적 [파쇄] 2' }),
  GH({ id: 'gh_vmaw',     enemyKey: 'voidMaw',        rank: 'cyan', name: '공허 아가리혼',  passive: { drainPct: 4 },      active: { power: { stat: 'str', pct: 30 }, drainPct: 30 }, aDesc: '완력 30% + 흡혈' }),
  GH({ id: 'gh_atitan',   enemyKey: 'abyssalTitan',   rank: 'cyan', name: '심연 거신혼',    passive: { hpPct: 8 },         active: { power: { stat: 'str', pct: 34 } }, aDesc: '완력 34% 타격' }),
  GH({ id: 'gh_mcore',    enemyKey: 'manticore',      rank: 'cyan', name: '만티코어 원혼',  passive: { crit: 7 },          active: { power: { stat: 'str', pct: 30 }, apply: [{ s: 'poison', n: 2 }] }, aDesc: '완력 30% + [중독] 2' }),
  GH({ id: 'gh_vnoble',   enemyKey: 'vampireNoble',   rank: 'cyan', name: '귀족의 갈증',    passive: { drainPct: 4 },      active: { power: { stat: 'int', pct: 30 }, drainPct: 40 }, aDesc: '지혜 30% + 흡혈' }),
  GH({ id: 'gh_scylla',   enemyKey: 'scyllaSpawn',    rank: 'cyan', name: '스킬라 촉수혼',  passive: { statusChance: 10 }, active: { apply: [{ s: 'bind', n: 2 }] },   aDesc: '적 [속박] 2' }),
  // ── 남령 +6 (최심층 강적 — 남령부 사용처 확대) ──
  GH({ id: 'gh_cyclops',  enemyKey: 'cyclopsWarden',  rank: 'indigo', name: '외눈 간수혼',   passive: { physPct: 11, takenPct: -4 }, active: { power: { stat: 'str', pct: 38 }, apply: [{ s: 'stun', n: 1 }] }, aDesc: '완력 38% + [기절] 1' }),
  GH({ id: 'gh_gdemon',   enemyKey: 'greaterDemon',   rank: 'indigo', name: '상급 마령',     passive: { magPct: 11, crit: 5 },       active: { power: { stat: 'int', pct: 40 }, apply: [{ s: 'burn', n: 2 }] }, aDesc: '지혜 40% + [화상] 2' }),
  GH({ id: 'gh_hecaton',  enemyKey: 'hecaton',        rank: 'indigo', name: '백수 거인혼',   passive: { physPct: 11, hpPct: 8 },     active: { power: { stat: 'str', pct: 22 }, apply: [{ s: 'shatter', n: 2 }] }, aDesc: '완력 22% 연격 + [파쇄] 2' }),
  GH({ id: 'gh_torturer', enemyKey: 'ancientTorturer',rank: 'indigo', name: '고문관의 악령', passive: { statusChance: 12, crit: 5 }, active: { apply: [{ s: 'bleed', n: 2 }, { s: 'bind', n: 1 }] }, aDesc: '적 [출혈] 2 + [속박] 1' }),
  GH({ id: 'gh_akaoni',   enemyKey: 'akaOni',         rank: 'indigo', name: '붉은 뿔귀령',   passive: { physPct: 11, drainPct: 4 },  active: { power: { stat: 'str', pct: 40 }, drainPct: 30 }, aDesc: '완력 40% + 흡혈' }),
  GH({ id: 'gh_surtr',    enemyKey: 'surtr',          rank: 'indigo', name: '수르트의 불씨', passive: { magPct: 11, hpPct: 8 },      active: { power: { stat: 'int', pct: 42 }, apply: [{ s: 'burn', n: 3 }] }, aDesc: '지혜 42% + [화상] 3' }),
];
