...
export const CHANGELOG = [
  {
    version: '1.63.1',
    date: '2026-06-15',
    label: '도감 적 탭 레이아웃 재설계 + 네트워크 캐시 최적화',
    changes: [
      { type: 'balance', text: '[적 탭 1열 풀폭 16:9] 1.63.0 2열 16:9가 카드 가로 150px×세로 84px로 얇은 띠처럼 보이던 문제 해결. 적 탭만 1열 풀폭으로 변경(타 탭은 2열 유지) → 카드 가로 ~350px×세로 ~197px로 일러 본연의 디테일과 미발견 자물쇠 가시성 확보. 자물쇠 크기도 18→32px로 확대' },
      { type: 'system', text: '[네트워크·디코딩 최적화] 적 일러 102개 동시 로드(약 25MB) 위험을 lazy/async로 해결. 도감 진입 시 가시 영역 5~6개만 즉시 로드(약 1.5MB), 스크롤 진행 시 점진 로드. 두 번째 방문 시 브라우저 HTTP 캐시로 0회 호출. PWA precache는 코어(1.3MB)만 유지하여 첫 설치 부담 그대로' },
      { type: 'system', text: '[모달 일러 eager] 카드 클릭 직후 모달 일러는 이미 브라우저 캐시에 있으므로 eager+async로 즉시 표시' },
    ],
  },
  {
    version: '1.63.0',
