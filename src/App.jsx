...
function calculateDamage(skill, attacker, defender, skills, isCrit, ultimates = [], meta = null, curses = []) {
...
function rollCrit(skills, attacker, meta = null) {
...
// =========== 전투 화면 ===========
function CombatScreen({ classData, initialPlayer, initialSkills, initialUltimates = [], enemyKey, isBoss, expedition, curses = [], meta, onVictory, onDefeat }) {
    ...
    if (skill.type === 'physical' || skill.type === 'magic') {
      ...
      for (let echo = 0; echo < echoTimes; echo++) {
        ...
        for (let i = 0; i < hitCount; i++) {
          let isCrit = rollCrit(skills, newPlayer, meta);
          ...
          const dmgResult = calculateDamage(skill, newPlayer, newEnemy, skills, isCrit, ultimates, meta, curses);
...
export default function App() {
  ...
  const handleVictory = (remainingHp, drop) => {
    ...
    let count = hasEffect(skills, 'extraReward') ? 4 : 3;
    ...
    <div className="min-h-screen flex items-center justify-center p-6" style={{
      ...
      <div className="grid lg:grid-cols-[1fr_auto_1fr] gap-8 items-start max-w-7xl">
        ...
        <div className="hidden lg:block max-w-sm" style={{ color: PALETTE.text }}>
          <p className="text-xs tracking-[0.4em] mb-2" style={{ color: PALETTE.deblan }}>EXPEDITION SYSTEM · v1.0</p>
          ...
          <p className="text-sm leading-relaxed mb-6" style={{ color: PALETTE.textDim }}>
            v1.0 — 원정 시스템 + 영혼의 제단 + 메타 진행.
          </p>
          
          <div className="space-y-3 text-xs">
            <div>
              <div className="text-[10px] tracking-[0.3em] mb-1.5" style={{ color: PALETTE.deblan }}>✦ 영혼의 제단</div>
              <p style={{ color: PALETTE.textDim }}>
                런이 끝나면 영혼 획득 → 메인 메뉴의 "★ 영혼의 제단"에서 영구 강화 구매.
                매번 랜덤 3개 슬롯, 영혼 1로 새로고침.
              </p>
            </div>
            <div>
              <div className="text-[10px] tracking-[0.3em] mb-1.5" style={{ color: PALETTE.derod }}>◆ 원정 시스템</div>
              <p style={{ color: PALETTE.textDim }}>
                4챕터 = 1원정. 원정 클리어 시 다음 원정 해금.
                상위 원정은 적 능력치 배율 + 저주 시스템으로 난이도 상승.
              </p>
            </div>
            <div>
              <div className="text-[10px] tracking-[0.3em] mb-1.5" style={{ color: PALETTE.legendary }}>★ 데로드의 사제</div>
              <p style={{ color: PALETTE.textDim }}>
                새 직업 (영혼 100으로 해금). 신성광선·축복·가호로 회복+버프 빌드.
              </p>
            </div>
          </div>

          <div className="mt-6 pt-5 border-t text-[11px] leading-relaxed" style={{ color: PALETTE.textDim, borderColor: PALETTE.panelBorder }}>
            ◇ 사망 시 누적 영혼의 70% 획득<br/>
            ◇ 원정 클리어 시 보너스 영혼 (30/60/120/250)<br/>
