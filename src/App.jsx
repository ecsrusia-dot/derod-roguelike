...
function rollRewards(count = 3, eliteBonus = false, skills = {}, relics = [], ultimates = [], currentClassId = null) {
...
function CombatScreen({ classData, initialPlayer, initialSkills, initialUltimates = [], initialRelics = [], activeSkills = null, activeRelicNames = null, enemyKey, isBoss, expedition, curses = [], meta, onVictory, onDefeat }) {
  ...
  const handlePlayerAction = (skillKey) => {
    ...
    // 정신의 화로 결계 보너스: 다음 마법 비용 0
    let mindForgeUsed = false;
    if (etherCost > 0 && skill.type === 'magic' && player.buffs?.mindForgeFreeNextMagic) {
      etherCost = 0;
      mindForgeUsed = true;
    }
    ...
    if (mindForgeUsed) {
      ...
      delete newPlayer.buffs.mindForgeFreeNextMagic;
      newLog.push({ type: 'passive', text: `★ [정신의 화로] 마법 비용 0` });
    }
    let newEnemy = { ...enemy };
    ...
    if (skill.type === 'physical' || skill.type === 'magic') {
      ...
      for (let echo = 0; echo < echoTimes; echo++) {
        ...
        for (let i = 0; i < hitCount; i++) {
          ...
          // === 이프리트 Lv.5: 치명타 적중 시 화염 각인 즉시 폭발 ===
          const ifritLvForExplode = (skills && skills['이프리트']) || 0;
          if (isCrit && ifritLvForExplode >= 5 && (!activeSkills || activeSkills.includes('이프리트')) && newEnemy.debuffs?.igniteDmg > 0 && newEnemy.debuffs?.igniteTurns > 0) {
            ...
            newLog.push({ type: 'crit', text: `🔥 [이프리트 Lv.5] 화염 폭발! ${explosionDmg} 데미지` });
            // 이프리트 강림 궁극: 폭발 후 즉시 재부여
            if (hasUltimate(ultimates, 'ult_ifritDescent')) {
              const newIgniteDmg = Math.floor(newPlayer.지능 * 0.3);
              const newIgniteTurns = ifritLvForExplode >= 7 ? 4 : 3;
              newEnemy.debuffs = { 
                ...newEnemy.debuffs, 
              ...
              newLog.push({ type: 'passive', text: `★ [이프리트 강림] 화염 각인 재부여 ${newIgniteDmg} ${newIgniteTurns}T` });
            } else {
              // 폭발 후 각인 소멸
              newEnemy.debuffs = { 
                ...newEnemy.debuffs, 
      ...
      // === 이프리트 화염 각인 (Lv.3 효과) ===
      // 마법 공격 시 30% 확률로 화염 각인 (이프리트 강림 +20%, 영겁 +30%)
      const ifritLvForIgnite = (skills && skills['이프리트']) || 0;
      if (skill.type === 'magic' && ifritLvForIgnite >= 3 && (!activeSkills || activeSkills.includes('이프리트')) && newEnemy.currentHp > 0) {
        let igniteChance = 0.3;
        if (hasUltimate(ultimates, 'ult_eternalFire')) igniteChance += 0.3;
        else if (hasUltimate(ultimates, 'ult_ifritDescent')) igniteChance += 0.2;
        if (Math.random() < igniteChance) {
          // 시전 시 지능으로 데미지 고정 (이후 변동 X)
          const igniteDmg = Math.floor(newPlayer.지능 * 0.3);
          // Lv.7 효과: 지속 +1턴 (3 → 4)
          let igniteTurns = ifritLvForIgnite >= 7 ? 4 : 3;
          // 정신의 화로 결계 후 보너스: turn +2
          if (newPlayer.buffs?.mindForgeIgniteBoost) {
            igniteTurns += 2;
            newPlayer.buffs = { ...newPlayer.buffs };
            delete newPlayer.buffs.mindForgeIgniteBoost;
          }
          // 영겁의 화염: 영구 (turns = 999 표시)
          const isEternal = hasUltimate(ultimates, 'ult_eternalFire');
          newEnemy.debuffs = { 
            ...newEnemy.debuffs, 
            igniteDmg: igniteDmg, 
            igniteTurns: isEternal ? 999 : igniteTurns,
            igniteEternal: isEternal,
          };
          newLog.push({ type: 'debuff', text: `🔥 [화염 각인] ${igniteDmg} 데미지 ${isEternal ? '영구' : igniteTurns + 'T'}` });
        }
      }
      // 자가 회복 (사제 - 신성광선)
      if (skill.selfHeal) {
    ...
    if (skill.type === 'defense') {
      ...
      // 정신의 화로: 결계 사용 시 다음 턴 마법 비용 0 + 화염 각인 +2턴 연장 보너스
      if (hasUltimate(ultimates, 'ult_mindForge') && skill.name === '결계') {
        newPlayer.buffs = { 
          ...newPlayer.buffs, 
          mindForgeFreeNextMagic: true,
          mindForgeIgniteBoost: true,
        };
        newLog.push({ type: 'passive', text: `★ [정신의 화로] 다음 마법 비용 0 + 화염 각인 +2턴` });
      }
    }
  ...
  const endTurn = (curPlayer, curEnemy, curLog) => {
    ...
    if (newEnemy.debuffs?.igniteDmg > 0 && newEnemy.debuffs?.igniteTurns > 0) {
      let igniteDmg = newEnemy.debuffs.igniteDmg;
      // 영겁의 화염: 데미지 +50%
      if (hasUltimate(ultimates, 'ult_eternalFire')) {
        igniteDmg = Math.floor(igniteDmg * 1.5);
      }
      // 화염 각인은 항상 방어 무시
      newEnemy.currentHp = Math.max(0, newEnemy.currentHp - igniteDmg);
      
      // 정신의 화로: 자기 회복 (데미지의 50%)
      if (hasUltimate(ultimates, 'ult_mindForge')) {
        const heal = Math.floor(igniteDmg * 0.5);
        if (heal > 0) {
          newPlayer.hp = Math.min(newPlayer.maxHp, newPlayer.hp + heal);
          newLog.push({ type: 'heal', text: `★ [정신의 화로] 화염 회수 HP +${heal}` });
        }
        // 매 턴 에테르 +1
        if (newPlayer.ether < newPlayer.maxEther) {
          newPlayer.ether = Math.min(newPlayer.maxEther, newPlayer.ether + 1);
          newLog.push({ type: 'passive', text: `★ [정신의 화로] 에테르 +1` });
        }
      }
...
// =========== 보상 선택 ===========
function RewardSelect({ rewards: initialRewards, gem, skills, relics, ultimates, activeSkills = null, onPick, onReroll, hasRerolled, isElite, classId = null }) {
  ...
  const handleReroll = () => {
    ...
    const newRewards = rollRewards(count, isElite, skills, relics, ultimates, classId);
...
export default function App() {
  ...
  const handleVictory = (remainingHp, drop) => {
    ...
    const rewards = rollRewards(count, isEliteReward, skills, relics, ultimates, classData?.id);
  ...
  const handleEventResolve = (resultData) => {
    if (resultData.reward) {
      if (resultData.reward.type === 'gold') setGold(prev => prev + resultData.reward.value);
        ...
        const relicRewards = getRewardPool(classData?.id).filter(r => r.type === 'relic' && !ownedNames.includes(r.name));
