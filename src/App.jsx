...
// 비활성 유물의 skillBonus를 차감한 effective skills 계산
// activeRelicNames: 활성 유물 이름 배열 (null이면 모두 활성)
function getEffectiveSkills(skills, relics, activeRelicNames = null) {
  if (!activeRelicNames) return skills;
  const result = { ...skills };
  relics.forEach(rel => {
    if (!rel.skillBonus) return;
    const isActive = activeRelicNames.includes(rel.name);
    if (isActive) return;
    // 비활성 유물의 보너스는 차감
    Object.entries(rel.skillBonus).forEach(([k, v]) => {
...
// chapterIdx: 원정 내 몇 번째 챕터인지 (0~3)
function generateChapterMap(chapter, chapterIdx = 0) {
  // 최소 4 layers: 시작(0) + 중간(1...) + 보스직전(layers-2) + 보스(layers-1)
  const layers = Math.max(4, Math.ceil(chapter.nodeCount / 2.5));
  const nodes = [];
  let id = 0;

  // Layer 0: 시작 노드
  // - 원정 첫 챕터(idx===0): 'prep' (전투 준비) 노드
  // - 그 외 챕터: 'rest' (정비) 노드
  const firstNodeType = chapterIdx === 0 ? 'prep' : 'rest';
  nodes.push({ id: id++, type: firstNodeType, layer: 0, x: 50, y: 95, completed: false, current: true, locked: false });
  ...
  // 보스 직전 레이어 (layers-2): 정비 노드 1개만 (모든 경로가 여기로 모임)
...
function calculateDamage(skill, attacker, defender, skills, isCrit, ultimates = [], meta = null, curses = [], activeSkills = null) {
...
function rollCrit(skills, attacker, meta = null, activeSkills = null) {
...
function rollDodge(skills, defender, activeSkills = null) {
...
function CombatScreen({ classData, initialPlayer, initialSkills, initialUltimates = [], initialRelics = [], activeSkills = null, activeRelicNames = null, enemyKey, isBoss, expedition, curses = [], meta, onVictory, onDefeat }) {
  ...
  // 전투 준비 봉인 시스템: 비활성 유물의 skillBonus 차감한 effective skills
    ...
    if (skill.type === 'physical' || skill.type === 'magic') {
      ...
      for (let echo = 0; echo < echoTimes; echo++) {
        ...
        for (let i = 0; i < hitCount; i++) {
          let isCrit = rollCrit(skills, newPlayer, meta, activeSkills);
          ...
          const dmgResult = calculateDamage(skill, newPlayer, newEnemy, skills, isCrit, ultimates, meta, curses, activeSkills);
  ...
  const executeEnemyTurn = (curPlayer, curEnemy, curLog) => {
    ...
    if (intent.type === 'attack') {
      const dodged = rollDodge(skills, newPlayer, activeSkills);
      if (dodged) {
    ...
    <div className="absolute inset-0 flex flex-col" style={{ background: PALETTE.bgDeep }}>
      ...
      <div className="flex-1 px-5 py-5 space-y-3 overflow-y-auto" style={{
        ...
        <button onClick={() => onChoice({ type: 'maxhp', value: GAME_CONFIG.rest.maxhpAmount })}
          ...
          <div className="text-[11px]" style={{ color: PALETTE.textDim }}>최대 체력 +{GAME_CONFIG.rest.maxhpAmount} (이번 회차 한정)</div>
        </button>
      </div>
    </div>
  );
...
function PrepScreen({ skills, relics, ultimates, expedition, onConfirm }) {
  // Lv > 0 인 보유 패시브 목록
  ...
  // 자동 통과면 모두 활성화
  const initialSelectedSkills = skillsAutoPass ? new Set(ownedSkills) : new Set();
  const initialSelectedRelics = relicsAutoPass ? new Set(relics.map(r => r.name)) : new Set();
  ...
  const canConfirm = (skillsAutoPass || selectedSkills.size === maxSkillSelect) &&
                     (relicsAutoPass || selectedRelics.size === maxRelicSelect || relics.length === 0);
    ...
    <div className="absolute inset-0 flex flex-col" style={{ background: PALETTE.bgDeep }}>
                  ...
                  <button key={i} onClick={() => toggleRelic(rel.name)}
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-[10px]" style={{ color: PALETTE.textDim }}>
                        {Object.entries(rel.skillBonus || {}).map(([k, v]) => `${k}+${v}`).join(', ')}
                      </span>
                    </div>
                  </button>
                );
      ...
      <div className="p-3 border-t" style={{ borderColor: PALETTE.panelBorder }}>
        <button onClick={() => onConfirm(Array.from(selectedSkills), Array.from(selectedRelics))}
          ...
          {canConfirm ? '여정 시작 ▸' : '필요 개수만큼 선택하세요'}
        </button>
      </div>
    </div>
  );
    ...
    if (idx === 0) {
      ...
      if (startRelicCount > 0) {
        ...
        for (let i = 0; i < Math.min(startRelicCount, shuffled.length); i++) {
          ...
          // 유물 효과를 시작 패시브에도 적용
          Object.entries(shuffled[i].skillBonus || {}).forEach(([k, v]) => {
            baseSkills[k] = Math.min((baseSkills[k] || 0) + v, PASSIVE_SKILLS[k].maxLv);
          });
        }
        setSkills({ ...baseSkills });
      }
    ...
    <div className="min-h-screen flex items-center justify-center p-6" style={{
      ...
      <div className="grid lg:grid-cols-[1fr_auto_1fr] gap-8 items-start max-w-7xl">
        ...
        <div className="hidden lg:block max-w-sm" style={{ color: PALETTE.text }}>
          <p className="text-xs tracking-[0.4em] mb-2" style={{ color: PALETTE.deblan }}>BATTLE PREP · v1.1</p>
          ...
          <p className="text-sm leading-relaxed mb-6" style={{ color: PALETTE.textDim }}>
            v1.1 — 전투 준비 시스템 + 정비 노드 + 야영지 정리.
          </p>
          
          <div className="space-y-3 text-xs">
            <div>
              <div className="text-[10px] tracking-[0.3em] mb-1.5" style={{ color: PALETTE.green }}>◇ 전투 준비 (첫 노드)</div>
              <p style={{ color: PALETTE.textDim }}>
                원정 시작 시 보유 패시브 中 5개 / 유물 中 N개 선택. 나머지는 봉인 (효과 0).
                활성 유물 개수: 원정 1=1개 / 2=2개 / 3=3개 / 4=4개.
              </p>
            </div>
            <div>
              <div className="text-[10px] tracking-[0.3em] mb-1.5" style={{ color: PALETTE.derod }}>◆ 정비 노드 (보스 직전)</div>
              <p style={{ color: PALETTE.textDim }}>
                휴식(HP 40%) / 명상(보석 +5) / 단련(이번 회차만 HP +20).
                패시브 강화 옵션은 제거.
              </p>
            </div>
            <div>
              <div className="text-[10px] tracking-[0.3em] mb-1.5" style={{ color: PALETTE.accent }}>◆ 최대 HP 체계 정리</div>
              <p style={{ color: PALETTE.textDim }}>
                모든 최대 HP 보너스(능력치 보상, 재생 minor, 단련) = 회차 한정.
                영구 증가는 영혼의 제단 "강인한 시작"만 가능.
              </p>
            </div>
          </div>

          <div className="mt-6 pt-5 border-t text-[11px] leading-relaxed" style={{ color: PALETTE.textDim, borderColor: PALETTE.panelBorder }}>
            ◇ 봉인된 패시브도 보상에선 정상 등장 (Lv 올림 가능)<br/>
            ◇ 야영지 노드는 그래프에서 제거됨<br/>
