...
function CombatScreen({ classData, initialPlayer, initialSkills, initialUltimates = [], initialRelics = [], activeSkills = null, activeRelicNames = null, enemyKey, isBoss, expedition, curses = [], meta, onVictory, onDefeat }) {
  ...
  const executeEnemyTurn = (curPlayer, curEnemy, curLog) => {
    ...
    if (intent.type === 'attack') {
      ...
      if ((skills['심안류'] || 0) > 0 || hasUltimate(ultimates, 'ult_counterMirror') 
          || hasUltimate(ultimates, 'ult_counterShock') || hasUltimate(ultimates, 'ult_counterShadow')) {
        // 반격 확률 계산
        let counterRate = 20;  // 기본
        const simanLv = skills['심안류'] || 0;
        if (simanLv >= 3) counterRate += 10;  // Lv.3: +10%
        // 궁극 보너스
        if (hasUltimate(ultimates, 'ult_counterMirror')) counterRate += 50;
        else if (hasUltimate(ultimates, 'ult_counterShock')) counterRate += 40;
        else if (hasUltimate(ultimates, 'ult_counterShadow')) counterRate += 40;
        
        // 회피했고 명경지수면: 다음 턴 반격 데미지 +50% 효과
        if (dodged && hasUltimate(ultimates, 'ult_counterMirror')) {
        ...
        if (Math.random() * 100 < counterRate) {
          ...
          if (hasUltimate(ultimates, 'ult_counterMirror') 
              || hasUltimate(ultimates, 'ult_counterShock') 
              || hasUltimate(ultimates, 'ult_counterShadow')) {
          ...
          // 무영검: 누적된 미스 보너스 적용
          if (hasUltimate(ultimates, 'ult_counterShadow') && newPlayer.buffs?.shadowCounterStack > 0) {
          ...
          if (simanLv >= 7 || hasUltimate(ultimates, 'ult_counterShock') 
              || hasUltimate(ultimates, 'ult_counterMirror') 
              || hasUltimate(ultimates, 'ult_counterShadow')) {
            // Lv.7만 있을 때
            if (simanLv >= 7) {
          ...
          // 검로일여: 반격 시 충격 게이지 +30
          if (hasUltimate(ultimates, 'ult_counterShock')) {
            const currentGauge = newEnemy.debuffs?.shockGauge || 0;
          ...
          // 명경지수: 회피 시 발생한 다음턴 반격 데미지 +50% 버프 처리
  ...
  const endTurn = (curPlayer, curEnemy, curLog) => {
    ...
    // 심안류 궁극 1턴 버프 정리 (적용된 후 1턴 = 본인 턴 시작 시 소비됨)
    // 본인 턴이 시작될 때 mirrorDodgeNext/shadowCritNext는 그 턴에 사용되고 다음 적턴에 사라짐
    // → 새 턴 시작 시 정리
    ...
    if (newPlayer.buffs?.guaranteedCritNext) {
      // 본인 공격에 사용되거나 다음 턴 시작 전 정리
      // 사용되지 않았다면 유지 (한 번만 발동되도록 위에서 false로 설정함)
    }
  ...
  return (
    <div className="absolute inset-0 flex flex-col" style={{ background: PALETTE.bgDeep }}>
      ...
      <div className="flex-1 flex flex-col min-h-0">
        ...
        {/* 1. 상단 1/3 : 적 캐릭터 모습 및 정보 */}
        <div className="flex-1 flex flex-col min-h-0 border-b" style={{ borderColor: PALETTE.panelBorder }}>
          {/* 적 캐릭터 모습 (미구현 흑백 처리) */}
          <div className="flex-1 relative overflow-hidden bg-[#0a0608] flex items-center justify-center grayscale">
             {/* 흑백 패턴 배경 효과 */}
...
function StartScreen({ classData, onContinue }) {
  ...
  return (
    <div className="absolute inset-0 overflow-hidden" 
      onClick={onContinue}
      style={{
        background: `radial-gradient(ellipse at center, ${classColor}25, ${PALETTE.bgDeep} 70%)`,
        cursor: 'pointer',
      }}>
      {/* 배경 이미지 */}
      ...
      <div style={{
        ...
        background: `radial-gradient(ellipse at center, transparent 25%, ${PALETTE.bgDeep}c0 90%),
                     linear-gradient(180deg, ${PALETTE.bgDeep}80 0%, transparent 30%, ${PALETTE.bgDeep}f0 100%)`,
        pointerEvents: 'none',
      }} />
      ...
      <div style={{
        ...
        background: `radial-gradient(circle, ${classColor}30 0%, transparent 40%)`,
      ...
      <div style={{
        ...
        background: `conic-gradient(from 0deg, transparent 0deg, ${classColor}40 45deg, transparent 90deg,
                                              transparent 180deg, ${classColor}30 225deg, transparent 270deg)`,
        animation: 'startSpin 12s linear infinite',
        opacity: 0.35,
        pointerEvents: 'none',
        mixBlendMode: 'screen',
      }} />
      ...
      <style>{`
        ...
        @keyframes startTitleGlow {
          0%, 100% { text-shadow: 0 0 30px ${classColor}, 0 0 60px ${classColor}80; }
          50% { text-shadow: 0 0 50px ${classColor}, 0 0 100px ${classColor}, 0 0 80px ${classColor}80; }
        }
      ...
      <div className="relative h-full flex flex-col items-center justify-center px-6 py-8 text-center">
        <div className="text-xs tracking-[0.5em] mb-3" style={{ 
          color: classColor, 
          opacity: 0.85,
          animation: 'startSubFade 1s ease-out 0.3s both',
        }}>━━ D E P A R T U R E ━━</div>
        
        <h2 className="text-7xl font-bold mb-3" style={{
          color: PALETTE.text, 
          fontFamily: '"Cinzel", serif',
          animation: 'startTitleAppear 1.2s ease-out, startTitleGlow 3s ease-in-out 1.2s infinite',
        }}>출정</h2>
        
        {classData && (
          <>
            <div className="text-base font-bold tracking-[0.2em] mt-6" style={{ 
              color: classColor,
              animation: 'startSubFade 1.2s ease-out 0.8s both',
            }}>
              {classData.name}
            </div>
            <div className="text-[10px] tracking-[0.4em] mt-1" style={{ 
              color: PALETTE.textDim,
              animation: 'startSubFade 1.2s ease-out 1s both',
            }}>
              {classData.sub}
            </div>
            <p className="text-xs italic mt-6 max-w-xs leading-relaxed" style={{ 
              color: PALETTE.text,
              animation: 'startSubFade 1.5s ease-out 1.4s both',
            }}>
              {classData.desc}
            </p>
          </>
        ...
        {/* 탭 안내 */}
        <div className="absolute bottom-12 text-[11px] tracking-[0.4em]" style={{
          color: PALETTE.text,
          animation: 'startTapHint 1.8s ease-in-out 2s infinite',
        }}>
          ▸ 화면을 탭하여 원정 선택
        </div>
      </div>
...
function VictoryScreen({ classData, enemy, onContinue }) {
  ...
  return (
    <div className="absolute inset-0 overflow-hidden" 
      onClick={onContinue}
      style={{
        background: `radial-gradient(ellipse at center, ${PALETTE.legendary}25, ${PALETTE.bgDeep} 70%)`,
        cursor: 'pointer',
      }}>
      {/* 배경 이미지 (직업별) */}
      ...
      <div style={{
        position: 'absolute', inset: 0,
        background: `radial-gradient(ellipse at center, transparent 30%, ${PALETTE.bgDeep}c0 90%), 
                     linear-gradient(180deg, transparent 40%, ${PALETTE.bgDeep}f0 100%)`,
        pointerEvents: 'none',
      }} />
      
      {/* 황금빛 빛줄기 (방사형) - CSS keyframe 애니메이션 */}
      <div style={{
        position: 'absolute',
        top: '50%', left: '50%',
        width: '200%', height: '200%',
        marginLeft: '-100%', marginTop: '-100%',
        background: `radial-gradient(circle, ${PALETTE.legendary}33 0%, transparent 35%)`,
      ...
      <div style={{
        ...
        background: `conic-gradient(from 0deg, transparent 0deg, ${PALETTE.legendary}40 30deg, transparent 60deg,
                                              transparent 120deg, ${PALETTE.legendary}30 150deg, transparent 180deg,
                                              transparent 240deg, ${PALETTE.legendary}40 270deg, transparent 300deg)`,
        animation: 'victorySpin 6s linear infinite',
        opacity: 0.4,
        pointerEvents: 'none',
        mixBlendMode: 'screen',
      }} />
      ...
      <style>{`
        ...
        @keyframes titleGlow {
          0%, 100% { text-shadow: 0 0 30px ${PALETTE.legendary}, 0 0 60px ${PALETTE.legendary}80; }
          50% { text-shadow: 0 0 50px ${PALETTE.legendary}, 0 0 100px ${PALETTE.legendary}, 0 0 80px ${PALETTE.derod}; }
        }
        @keyframes subFadeIn {
      ...
      <div className="relative h-full flex flex-col items-center justify-center px-6 py-8 text-center">
        <div className="text-xs tracking-[0.5em] mb-3" style={{ 
          ...
          opacity: 0.85,
          animation: 'subFadeIn 1s ease-out 0.3s both',
        }}>━━ V I C T O R Y ━━</div>
        
        <h2 className="text-6xl font-bold mb-3" style={{
          color: PALETTE.text, 
          fontFamily: '"Cinzel", serif',
          letterSpacing: '0.1em',
          animation: 'titleSlideIn 0.8s ease-out, titleGlow 2.5s ease-in-out 0.8s infinite',
        }}>승리</h2>
        
        <p className="text-sm italic mt-4 mb-2" style={{ 
          color: PALETTE.textDim,
          animation: 'subFadeIn 1.2s ease-out 0.6s both',
        }}>
        ...
        {/* 직업명 표시 */}
        {classData && (
          <div className="text-[10px] tracking-[0.3em] mt-1" style={{ 
            color: classData.color, 
            opacity: 0.8,
            animation: 'subFadeIn 1.2s ease-out 0.9s both',
          }}>
            {classData.name}
          </div>
        )}
        ...
        <div className="absolute bottom-12 text-[11px] tracking-[0.4em]" style={{
          color: PALETTE.text,
          animation: 'tapHint 1.8s ease-in-out 1.5s infinite',
        }}>
          ▸ 화면을 탭하여 계속
        </div>
      </div>
...
function DefeatScreen({ classData, chapter, soulsGained, onContinue }) {
  ...
  // 화면 진입 시 1회 흔들림 (CSS 애니메이션은 자동으로 한 번만)
  useEffect(() => {
  ...
  return (
    <div className="absolute inset-0 overflow-hidden" style={{
      ...
      <div style={{
        position: 'absolute', inset: 0,
        background: `radial-gradient(ellipse at center, transparent 15%, ${PALETTE.bgDeep}d0 80%, ${PALETTE.bgDeep} 100%),
                     linear-gradient(180deg, ${PALETTE.bgDeep}80 0%, transparent 30%, ${PALETTE.bgDeep}f0 100%)`,
        pointerEvents: 'none',
      }} />
      ...
      <div style={{
        ...
        background: `linear-gradient(180deg, transparent 0%, ${PALETTE.accent}15 60%, ${PALETTE.accent}25 100%)`,
        animation: 'defeatMistRise 4s ease-out',
        pointerEvents: 'none',
      }} />
      ...
      <div style={{
        ...
        background: `radial-gradient(circle, ${PALETTE.accent}25 0%, transparent 50%)`,
      ...
      <style>{`
        ...
        @keyframes defeatGlow {
          0%, 100% { text-shadow: 0 0 30px ${PALETTE.accent}, 0 0 60px ${PALETTE.accent}80; }
          50% { text-shadow: 0 0 50px ${PALETTE.accent}, 0 0 90px ${PALETTE.accent}, 0 0 120px ${PALETTE.accent}80; }
        }
      ...
      <div className="relative h-full flex flex-col items-center justify-center px-6 py-8 text-center">
        ...
        <h2 className="text-6xl font-bold mb-3" style={{
          ...
          fontFamily: '"Cinzel", serif',
          letterSpacing: '0.1em',
          animation: 'defeatTitleAppear 1.2s ease-out, defeatGlow 3s ease-in-out 1.2s infinite',
        }}>죽음</h2>
        
        <p className="text-xs italic mt-2 mb-6" style={{ 
          color: PALETTE.textDim,
        ...
        <p className="text-sm leading-relaxed italic max-w-xs" style={{ 
          color: PALETTE.text,
        ...
        {/* 영혼 회수 카운터 */}
        <div className="mt-8 px-8 py-4 flex flex-col items-center" style={{
          background: `${PALETTE.deblan}25`,
          border: `1px solid ${PALETTE.deblan}80`,
        ...
        <button onClick={onContinue} className="mt-8 px-12 py-3" style={{
          background: `linear-gradient(180deg, ${PALETTE.accent}40, ${PALETTE.accentDim}40)`,
          border: `1px solid ${PALETTE.accent}`,
          color: PALETTE.text, letterSpacing: '0.3em', fontSize: '14px',
          animation: 'defeatSubFade 1.5s ease-out 2.2s both',
        }}>▸ 메인 메뉴</button>
      </div>
    </div>
  );
...
export default function App() {
  ...
  // 승리 화면 후 이동할 다음 화면 ('reward' | 'chapterClear')
      ...
      <div className="flex flex-row items-center justify-center gap-12 w-full max-w-[1400px]">
        ...
        <div className="flex-shrink-0">
          <PhoneFrame>
            ...
            {screen === 'expeditionSelect' && <ExpeditionSelect meta={meta} onSelect={startExpedition} onBack={() => setScreen('start')} />}
