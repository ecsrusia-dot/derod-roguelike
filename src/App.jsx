...
function rollRewards(count = 3, eliteBonus = false, skills = {}, relics = [], ultimates = [], currentClassId = null, meta = null, expedition = null) {
  ...
  const filteredPool = [...REWARD_POOL, ...unlockedChampRelics].filter(r => {
    ...
    if (r.type === 'skill') {
      ...
      if (r.name === '심안류') {
        const simanUltimates = ULTIMATE_SKILLS['심안류'] || [];
        const hasAnySimanUlt = simanUltimates.some(u => ultimates.includes(u.id));
    ...
    if (intent.type === 'attack') {
      ...
      if (dodged) {
        ...
        if (newPlayer.firstHitImmune) {
          ...
          // 심안류 Lv.5: 반격 확률 사전 굴림 (성공 시 받는 데미지 50% 차단)
      ...
      if (simanLv > 0 || hasAnyCounterUlt) {
        ...
        if (rolledValue < counterRate) {
          ...
          // minor: 반격 데미지 +7%/Lv
          if (simanLv > 0) counterDmg = Math.floor(counterDmg * (1 + simanLv * 0.07));
          // Lv.5: 반격 데미지 +20%
