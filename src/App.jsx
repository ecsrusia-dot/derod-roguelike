          ...
          <div className="flex-1 px-2.5 py-1.5 overflow-y-auto" style={{
            ...
            <div className="grid grid-cols-2 gap-x-2 gap-y-0 text-[9px] mb-1">
              <div className="flex justify-between" style={{ color: PALETTE.textDim }}>
                <span>근력</span><span className="font-bold tabular-nums" style={{ color: PALETTE.text }}>{player.근력}</span>
              </div>
              <div className="flex justify-between" style={{ color: PALETTE.textDim }}>
                <span>민첩</span><span className="font-bold tabular-nums" style={{ color: PALETTE.text }}>{player.민첩}</span>
              </div>
              <div className="flex justify-between" style={{ color: PALETTE.textDim }}>
                <span>지능</span><span className="font-bold tabular-nums" style={{ color: PALETTE.text }}>{player.지능}</span>
              </div>
              <div className="flex justify-between" style={{ color: PALETTE.textDim }}>
                <span>매력</span><span className="font-bold tabular-nums" style={{ color: PALETTE.text }}>{player.매력}</span>
              </div>
            </div>
            ...
            {(() => {
              // 치명타율
              let critRate = 5 + Math.max(0, (player.민첩 - 10) * 0.5);
              critRate += getMinorBonus(skills, 'critRate+', activeSkills);
              critRate += getMetaBonus(meta, 'critRate+3%') * 3;
              critRate += relicStat.critRate || 0;
              ...
              // 회피율
              let dodgeRate = Math.max(0, (player.민첩 - 10) * 0.3);
              dodgeRate += getMinorBonus(skills, 'dodge+', activeSkills);
              dodgeRate += relicStat.dodge || 0;
            ...
            {relics && relics.length > 0 && (
              <>
                ...
                <div className="flex flex-wrap gap-0.5">
                  {relics.filter(r => !activeRelicNames || activeRelicNames.includes(r.name)).map((rel, i) => (
