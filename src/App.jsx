    ...
    <div className="absolute inset-0 flex flex-col" style={{ background: PALETTE.bgDeep }}>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-3 space-y-3">
        ...
        <div className="text-[9px] text-center mt-3 px-3 py-2" style={{ 
          ...
          ⚠ Phase 1 — 챕터 구성과 적은 다음 업데이트에서 추가됩니다.<br/>
          현재는 시스템 골격만 구현되어 있습니다.
        </div>
      </div>
      )}
      ...
      if (dodged) {
        ...
        if (newPlayer.firstHitImmune) {
          ...
          if (newEnemy.berserkStacks > 0) {
            dmg += newEnemy.berserkStacks;
          }
          const takenBreakdown = [`기본 ${dmg}`];
    ...
    if (newPlayer.debuffs?.stunnedTurns > 0) {
      ...
      setTimeout(() => {
        setPlayer(prev => ({
          ...
          debuffs: {
            ...prev.debuffs,
            stunnedTurns: Math.max(0, (prev.debuffs?.stunnedTurns || 0) - 1),
          }
        }));
        setLog(prev => [...prev, { type: 'debuff', text: `💫 기절 — 행동 불가 (턴 스킵)` }]);
        // 기절 시 phase는 enemyTurn으로 (다시 적 공격)
        setPhase('enemyTurn');
        actionLockRef.current = false;
      }, 500);
      return;
...
export default function App() {
  ...
  const initializeRun = (chapterData, idx = 0, expeditionOverride = null, cursesOverride = null) => {
    ...
    if (idx === 0) {
      ...
      // 챔피언십 클리어로 해금된 유물 자동 보너스 (한기의 결정 등)
      // 해당 챔피언십 진입 시 해당 유물 자동 시작 보너스로 부여
      // (예: 북부 클리어 후 → 다음 북부 도전 시 한기의 결정 자동 보유)
      if (exp?.isChampionship && meta.championshipRelicUnlocks?.length > 0) {
        const champId = exp.championshipId;
        const matchingRelics = RELICS.filter(r => 
          r.championshipUnlock === champId && 
          meta.championshipRelicUnlocks.includes(r.name)
        );
        for (const relic of matchingRelics) {
          const relicReward = { type: 'relic', ...relic };
          startRelics.push(relicReward);
        }
      }
      
      if (startRelicCount > 0) {
        // weight 0 유물 (챔피언십 전용 유물) 제외 — 자동 시작 보너스로만 등장
        const randomablePool = RELICS.filter(r => (r.weight || 0) > 0);
        const shuffled = [...randomablePool].sort(() => Math.random() - 0.5);
  ...
  const handleChapterContinue = () => {
    ...
    if (isLastChapter) {
      ...
      if (currentExpedition.isChampionship) {
        ...
        // 챔피언십 ID = 유물의 championshipUnlock과 매칭
        if (wasFirstClear) {
          const relicMap = {
            frost: '한기의 결정',
            forest: '광기의 송곳니',  // Phase 2-3 이후 추가 예정
            sanctum: '봉인의 인장',
            rift: '균열의 핵',
            dawn: '여명의 성배',
          };
          ...
          if (relicName) {
            // 해당 유물이 RELICS에 정의되어 있을 때만 해금 (정의 안 된 유물은 무시)
