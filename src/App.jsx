  ...
  return (
    <div className="absolute inset-0 flex flex-col" style={{ background: PALETTE.bgDeep }}>
      ...
      <div className="flex-1 flex flex-col min-h-0">
        ...
        {/* 3. HP 정보창 — 일러스트와 완전 분리 (덮어쓰지 않음) */}
        <div className="shrink-0 px-3 py-2 border-t" style={{ 
          borderColor: PALETTE.panelBorder,
          background: `linear-gradient(90deg, ${classData.color}20, ${PALETTE.bgDeep})`,
        }}>
          <div className="flex justify-between items-center mb-1">
            <span className="text-xs font-bold" style={{ color: classData.color }}>{classData.name}</span>
            <span className="text-[11px] tabular-nums font-bold" style={{ color: PALETTE.text }}>
          ...
          {/* 버프/상태 영역 — 항상 고정 높이로 일러 흔들림 방지 */}
          <div className="flex items-center gap-1.5 flex-wrap min-h-[18px]">
            <span className="text-[9px] px-1.5 py-0.5" style={{ background: `${PALETTE.deblan}40`, color: '#fff', border: `1px solid ${PALETTE.deblan}80` }}>
              ✦ 에테르 {player.ether}/{player.maxEther}
            </span>
            {player.defense > 0 && (
              <span className="text-[9px] px-1.5 py-0.5" style={{ background: `${PALETTE.defense}40`, color: '#fff', border: `1px solid ${PALETTE.defense}80` }}>
                ◈ 방어 {player.defense}
              </span>
            )}
            {player.buffs?.rage > 0 && (
              <span className="text-[9px] px-1.5 py-0.5" style={{ background: `${PALETTE.accent}40`, color: '#fff', border: `1px solid ${PALETTE.accent}80` }}>
                ☩ 분노 ({player.buffs.rage}T)
              </span>
            )}
            {player.firstHitImmune && (
              <span className="text-[9px] px-1.5 py-0.5" style={{ background: `${PALETTE.legendary}40`, color: '#fff', border: `1px solid ${PALETTE.legendary}80` }}>
                ✦ 무적 1회
              </span>
            )}
          </div>
        ...
        <div className="shrink-0 h-48 relative overflow-hidden border-t" style={{ borderColor: PALETTE.panelBorder }}>
          ...
          />
          {/* 하단 가독성용 살짝 어두움 */}
          <div className="absolute inset-x-0 bottom-0 h-12 bg-gradient-to-t from-[#050304]/60 to-transparent" />
        </div>
