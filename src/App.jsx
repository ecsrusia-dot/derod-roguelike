...
export default function App() {
  ...
  const handleBuriedStart = (classId, dungeonId = 'labyrinth', contracts = [], startFloor = 1, raceId = null, keystones = []) => {
    setBuriedForgeNotice(null); // 이전 캐릭터의 정산·해금 알림 제거 (1.117.0)
    setMeta(prev => {
      const b = getBuried(prev);
      const char = createBuriedChar(classId, { items: [], gold: b.legacyGold }, dungeonId, contracts, aggregateBuriedParts(b.parts), startFloor, buriedEarnedDepthTraits(b.deepestByDungeon), raceId, keystones);
      if (!char) return prev;
