...
function rollRewards(count = 3, eliteBonus = false, skills = {}, relics = [], ultimates = [], currentClassId = null, meta = null, expedition = null) {
...
function TitleScreen({ meta, onStart, onAltar, onAchievements }) {
  return (
    <div className="absolute inset-0 flex flex-col items-center justify-between py-12 px-8" style={{
      ...
      {/* 버전 정보 (하단 작은 텍스트) */}
      <div className="absolute bottom-2 left-0 right-0 text-center" style={{ pointerEvents: 'none' }}>
        <div className="text-[9px] tracking-[0.3em]" style={{ color: PALETTE.textDim, opacity: 0.5 }}>
          v{GAME_VERSION} · {VERSION_LABEL} · {VERSION_DATE}
        </div>
      </div>
    </div>
  );
                    ...
                    {isDiscovered ? (
                      <>
                        <div className="flex items-center gap-2 text-[11px]">
                          <span style={{ color: PALETTE.text }}>{recipe.materials[0]}</span>
                          <span style={{ color: PALETTE.textDim }}>+</span>
                          <span style={{ color: PALETTE.text }}>{recipe.materials[1]}</span>
                        </div>
...
// =========== 챕터 클리어 ===========
function ChapterClearScreen({ chapter, isLastChapter, hp, maxHp, meta, curses = [], onContinue }) {
