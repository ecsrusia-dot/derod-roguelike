...
| 현재 게임 버전 | **1.121.0** (⚰ 무덤의 유산 시리즈: 1.103 신설 → 1.104 원작 확장 → 1.105 재련소 → 1.106 유니크 30+방벽 → 1.107 이벤트 방+접두어 → 1.108 저주 18종 → 1.109 조우 직업 3종 → 1.110 유니크 60종 완성 → 1.111 마의 계약 18종(출정 지참 2, 영구 소장) → 1.112 연구실 부품 16종+☠죽음의 조각(보스 드랍)+🌑재앙 소환(이벤트 방 게이지 5, 유니크 확정·같은 층 복귀·기울기 0.16 시뮬 검증) → 1.113 난이도 전면 개편(PM 지시 5건: **층 무한**(정복=구 최종층 보스 처치, 런 계속, buriedBossKeyAt 로테이션) / **레벨 스탯 3p·HP18 폐지**(성장 100% 장비, 레벨업=전액회복만) / **장비 레벨 Lv 표시+옵션도 레벨 배율**(확률형 ×2.2 상한) / **굴림 폭 0.75~1.20, 전설은 고정 굴림** / **인벤토리 폐지**(획득 즉시 교체/버리기 → 자동 분해, resolveBuriedLoot+BuriedLootModal, 구세이브 가방은 loadMeta에서 자동 분해). ⚠️ 무한층 밸런스 핵심: 드랍 장비 레벨=마물 레벨(파리티)이라 **depthPressurePerFloor(BURIED_TUNING)** 없으면 무한 스노볼. 1.114에서 복리 3% → **선형 1%/층** 재조정 — 복리는 100층 적 14배로 체크포인트(PM 지시)가 영원히 불가능했음. 현행: 100층 ×1.9 / 사망 중앙값 ~112-132층) → 1.114~1.116 던전 정체성 3부작(PM 지시 "던전은 난이도 아닌 정체성으로": **기믹 4종**(maze 선택지 3-4 / flood 도트 ×1.5·회복 -25% / chute HP 25% 지불 3층 낙하 / dark 적 수치 은폐 — dungeon.gimmick 필드, 전투는 gimmickId 분기) + **100층 체크포인트**(buriedCheckpointFloors, 시작 장비 = 마물 레벨 + 압력 보정 낡은 6종) + **던전 전용 유니크 16종**(BURIED_UNIQUES dungeon 필드, 심층 보스만·미보유 50% 우선) + **전용 부품 8종**(needDeep 100 해금) + **심층 직업 4종**(BURIED_DEPTH_CLASSES, 100층+ 보스 처치 해금) + **심층 특성 4종**(BURIED_DEPTH_TRAITS 150층 기록 해금, char.depthTraits로 생성 시 구움 — buriedTraitIds가 병합)) → 1.117 설명 팝업+계승 폐지+전면 점검(①상태·기믹·층·방 칩 탭 → BuriedInfoModal ②장비 계승 폐지: buriedDeathSettlement가 장착+대기 장비 전부 분해 → 먼지(레벨 반영), 골드 30%만 legacyGold — 보관함 UI·expandBuriedLegacy 삭제, 구세이브 보관함 loadMeta 자동 정산 ③감사 에이전트 2기 31건 중 30건 픽스 — 치명: 새로고침 도주(던전 마운트 시 char.room 전투면 강제 재전투)/재련소 레벨=층수 파워브레이크(buriedForgeLevel 마물 레벨 환산)/전설 tierWeightAt 유출/체크포인트 보정식 오산((압력-1)×기저배율/0.14)/int 기본기 침묵 무력화(stat 없으면 max(atk,fin,mag))/적 골드 미스케일(심층 경제 사망). ⚠️ 재발 방지 룰: 랜덤 롤·setState는 setMeta updater 밖(StrictMode 이중 실행), 알림은 buriedForgeNotice → 던전 화면 notice prop으로도 전달됨) → 1.118 SP 개편(패시브 회복 3+int/12 — 기본기·SP스킬이 주 엔진, 가동률 100%→50% 시뮬)+사망 골드 전액 소멸(legacyGold 신규 누적 중단) → 1.119 로비 허브+4단계 출정 위저드(BuriedScreen view state: home/wizard/forge/contracts/lab/records — 체크포인트 없으면 층 단계 자동 스킵)+전투 이펙트(index.css fx-b-* 7종: 슬래시/버스트/치명 링+펀치/셰이크/피격 비네트/회복 글로우 — pushFloat 부호 자동 트리거 + reduced-motion 가드, Playwright e2e 스크린샷 검증) → 1.120 적 풀 38종(신규 26 — dungeons 필드로 던전 전용 배정, buildBuriedRoomEnemy 필터)+층계 수문장(BURIED_GUARDIAN_KEYS 3종 로테이션, buriedBossKeyAt floor%100 우선, 재앙 HP 7배, 조각 8배+유니크 확정, 체크포인트는 격파(F+1 도달) 시 해금·재출발 F+1층)+난이도 계단(depthStep50 1.5·depthStep100 2 — 기존 선형과 곱, 100층 ×5.7/200층 ×26) → 1.121 심층 경제 재설계(⚠️ Lv.1532 사태 교훈: **압력을 장비 레벨로 환산 금지** — 레벨 인플레·드랍 무쓸모·방어 감쇠 붕괴 3연쇄. 현행: 장비 레벨=마물 레벨 고정, 압력은 buriedGearPower(=압력÷깊이격차 depthEdgePerFloor 0.008) **위력 배율(powerMult)**로 rollBuriedItem 스탯에만 — 시작·드랍·상점·부장품·협상·관·재련소·유니크 전 생성처 공유. 확률형 옵션은 powerMult 제외. 방어 감쇠 하한 defMult ≥0.25. 시뮬: 201층 재출발 드랍=시작 장비 149%·3합 버팀, 조직 런은 100층 수문장이 벽). **원전: docs/buried-reference.md** — PM 제공 나무위키 7문서 요약, 유니크 113·직업 70·저주 72 등 미이식 원전 포함. PM 승인 4패키지 전부 출시 완료) → 1.122 종족 시스템(BB2 모티브 — `BURIED_RACES` 10종: statMods 기본 스탯 가감 + fx(특성·계약과 동일 어휘, `buriedRaceFx`가 rf bag으로 buriedDerived·전투 병합). createBuriedChar 8번째 인자 raceId, 위저드 5단계(던전→층→종족→직업→최종), 구 캐릭터 raceId 없음 → 빈 fx 회귀 안전. PM 시트 업로드 완료 → 1.123 보정 완료) → 1.123 ᚱ룬 소켓(BB2 시트 이식 2탄 — `BURIED_RUNES` 16종 ★~★★★★, fx 어휘는 접두어(BURIED_MODS)와 100% 동일·`applyBuriedSkillFx` 공용 적용, `buriedModdedSkill(skill, modId, runeId)` 3인자 확장. 드랍: 전투 승리 일반6%/강적22%/보스40%/수문장·재앙100% → `result.rune` → App이 char.runes 주머니 적재. 각인: BuriedManage 룬 주머니 → `socketBuriedRune` 영구(제거 불가, 장비 버리면 소멸 — 도박 룰), 장비당 소켓 1칸(`item.rune`). + 종족 statMods 시트(種族Race) 정밀 보정 — 엘프 barrier15·하플링 int-4·용인 str5/vit2/dex-4·페어리 str-3·뱀파이어 dex형. ⚠️ BB2 시트 원본: /root/.claude/uploads .../a29e2e4d-*.xlsx — 22시트(스킬 421·몬스터 1181·계약 176·유니온·楔石·전설무구 등 미이식 원전 다수)) → 1.124 ☠엘리트 변형(BB2 시트 이식 3탄, PM 선택 — `BURIED_ELITE_MODS` 12종: 강적 방 적에 랜덤 접두어. 스탯 변형(hpMult/atkMult/defMult/extraHit/addApply)은 `applyBuriedEliteMod`가 생성 시점에 굽고(보상 ×1.4 포함), 판정 변형(immuneCrit/enrage/undying/critAdd/takenPct/barrierPct)은 전투가 enemy.eliteFx로 소비 — foe init 병합 + resolveBuriedAttack(def.immuneCrit·att.enrage) + eliteRevive(사망 체크 3곳 선행). buildBuriedRoomEnemy가 roomType 'elite'에서 항상 적용, 일반·보스는 무변형) → 1.125 경제 개편(PM 지시 3건 — ①강화 폐지: buriedEnhanceMult 삭제·구 +N 장비 배율 무효, ②골드 용처 신설: 제단 봉헌 3종(BURIED_ALTAR_BOONS — 수호 carryBarrier/축복 pendingStatuses/정화 저주 해제, 제단당 1회·깊이 비례 비용) + 상점 리롤(buriedShopRerollCost 반복 2배), ③재련소 400먼지·유물 8%·랜덤 60 + 수입 너프 BURIED_TUNING.goldEarnMult 0.7/dustEarnMult 0.65 — 재화 체감 조정은 이 두 상수) → 1.126 스킬 풀 확장(BB2 스킬 시트 이식 4탄 — BURIED_SKILLS 49→79종: 10개 라인 ×2 + 공용 10. 기존 필드 어휘만 사용(신규 메커니즘 0) — 전투·룬·접두어·스킬레벨 파이프라인 코드 변경 0줄. gear 이름 유일성 필수(장비명 생성)) → 1.127 ⚔전설무구 20종(BB2 전설무구 시트 이식 5탄 — BURIED_UNIQUES 76→96: **선언형 fx bag 신설** `buriedUniqueFx`(uf) — fx 필드 있는 유니크만 합산, buriedDerived + 전투 소비 5곳(statusChance/statusResist/gold/exp/luck/takenPct) 병합. 구 유니크 76종은 fx 없음 → 기존 uq() 분기 그대로 회귀 0. rune/mod 필드로 **룬 내장 드랍** (rollBuriedUniqueItem 프리셋). 신규 유니크는 이제 분기 코드 없이 fx 데이터만으로 추가 가능) → 1.128 ⚓쐐기석(BB2 楔石 이식 6탄, PM 결정 3건: 정복 던전만 개방·재화 +12%/P + 드랍 운 P÷3·최대 3개 — `BURIED_KEYSTONES` 12종(★1~3) + `buriedKeystoneFx`(kf bag)/`buriedKeystoneBonus`. createBuriedChar 9번째 인자 keystones(생성 시 구움), 위저드 6단계(직업 다음 쐐기석 — 미정복 던전 스킵). 소비: noDodge=buriedEffDodge/noCrit=resolveBuriedAttack/noAcc=derived·EquippedSkills·UniqueIds 3축/takenPct·noBarrier·startConfuse·cdAdd·noPotion·noHeal·darkAll=전투/noCampHeal=던전 rest·shrineHeal. 먼지 보너스는 buriedDeathSettlement·resolveBuriedLoot) → 1.129 📦보급 계약(BB2 契約 이식 7탄 — BURIED_CONTRACTS 18→28: supply 필드 {potions/gold/rune:[min,max]/item:tier/skillLv} — createBuriedChar 말미에서 지참분 지급(파리티 floor·powerMult), fx는 빈 객체라 aggregateBuriedContracts 오염 0. rollBuriedRuneIn(minRar,maxRar) 헬퍼 신설) → 1.130 👹몬스터 38→57(BB2 몬스터 시트 이식 8탄 — 신규 19종(공용 7 + 던전 전용 12, 일러 재사용) + **내성 축**: 적 필드 physTakenPct/magTakenPct(음수=내성)·fullGuardPct(전쾌시 방어)·startBarrier(실드형, buriedEnemyAtLevel이 m 스케일)·dodge/crit 개별치 — resolveBuriedAttack def-side 판정(기본기는 최고 스탯으로 물리/마법 라우팅), foe init 복사, 전투 시작 로그 내성 힌트(darkBlind 시 은폐). 시트 잔여 0) → 1.131 🌾출신 8종(BB2 出自 이식 9탄 — **시리즈 완주**. BURIED_ORIGINS: statMods+fx+startGold, PM 결정 「종족 화면 하단 통합」으로 위저드 6단계 유지. `buriedRaceFx`가 종족+출신을 하나의 rf bag으로 합산(hpMult 곱산) — 소비 지점 추가 0, rf.def만 derived def 라인에 신규 합산. createBuriedChar 10번째 인자 originId. 생성 3축 = 직업 17 × 종족 10 × 출신 8 = 1,360 조합 → 1.131.1 이력 초기화(storage.resetBuried — meta.buried만 EMPTY_BURIED 교체, 기록 화면 위험 구역 2단 확인. 클라우드는 lastSavedAt 최신 우선 전체 교체라 초기화 유지됨) → 1.132 ⛓스킬 사용 횟수(PM 지시 4건 — ①야영 무한 휴식 픽스: roomData.rested 방당 1회 ②스킬 레벨 종류별 재설계: buriedSkillAt kind 분기(공격 +8%/Lv·상태이상 스택·회복 +10%/Lv·SP +8%/Lv) ③BURIED_SKILL_USES D30/C22/B15/A10 ×(1+Lv×8%) — 잔여 = max - item.usesSpent(구세이브 undefined=만충), 소진 시 스킬만 봉인. 전투 spentMap state → result.usesSpent → App 정산 ④충전 3종: 야영 30%(rechargeBuriedUses)+봉헌 보충(만충 130골드)+상점 정비(만충·상점당 1회). ⚠️ 밸런스는 BURIED_SKILL_USES 한 곳) → 1.133 단품 충전+%전설 → 1.134 ⛓장비 파손+이벤트 기록 → 1.135 📜계약 이중 게이트(비용 누진 60×n + 보유 한도 진행도 연동) → 1.136 ⚡신속(PM 지시 "버프 쓰면 한 턴 날린다" — **power·heal 없고 SP 순증 아닌 순수 버프·디버프 22종 턴 미소모**, 물약처럼 턴당 1회(swiftUsedThisTurn). `swift` 필드는 BURIED_SKILLS 정의 직후 자동 파생 루프 — 신규 스킬도 규칙만 지키면 자동. 회복·집중·관찰은 제외(회복 루프·SP 붕괴 방지), 혼란 실패 시엔 턴 소모. 신속 사용 후 같은 턴 round-end가 나중에 오므로 cd+1 부기 그대로 정합. 1.136.1 「⚡신속」 전용 배지 — skillKindMeta가 swift 최우선 판정) → 1.137 ⚔유니크 원전 완성(BB1 도감 [1]~[113] 113종 완성 — 잔여 53종 이식: fx 선언형 21 + 전투 분기 29 + 던전 분기 3(변모 pendingStatuses·건강한 잠 야영 2배·한달음 stepBuriedChar 걸음 50%). 효과 기록 없던 15종은 오리지널 각색(src 번호 유지). 총 149종 = u113 + 던전 전용 16 + 전설무구 lg20. ✅ BB 원전은 **구글 드라이브 「내 드라이브/buried bornes」 폴더 상주** (BB2 공식 xlsx 22시트 + BB1 나무위키 mhtml 6건) — Google Drive MCP search_files로 접근, 세션 소실 문제 해결) → 1.138 스킬 2차(BB2 스킬 시트 79→119종: 라인당 +2 + 공용 +20. **⚡신속 공격기 5종**(잽·수리검 투척·서든 거스트·퀵 샷·베놈 샷) — 원전 Quick 플래그 정합, 공격 스킬 명시 `swift: true`(자동 파생과 별개). 원전 P(선제발동)는 플레이어 상시 선공이라 미이식. ⚠️ BURIED_SKILLS는 객체라 **id 충돌 시 조용히 덮어씀** — 신규 스킬 추가 후 반드시 node로 id·gear·name 중복 검증) → 1.139 몬스터 2차(BB2 몬스터 시트 57→80종: 공용 11 + 던전 전용 12, 내성 축 적극 활용 — 물리 벽·마법 내성·회피형·실드형) → 1.140 전설무구 2차(伝説の武具 시트 lg21~lg40 — 전부 선언형 fx, 룬 내장 6종. lg7 「그람」→「그람의 파편」 동명 정리. 유니크 총 169종) → 1.141 🏛조직(BB2 유니온 솔로 각색, PM 결정 「평판형·직업/종족 해금 포함」 — `BURIED_UNIONS` 4개(던전 1:1) + `meta.buried.unionRep/unlockedRaces` + `addBuriedUnionRep`(레벨 도달 보상 원자 지급: Lv5 전속 종족·Lv7 전속 직업) + App 승리 정산 훅(BURIED_UNION_REP_GAIN). 전속 직업 4종 `BURIED_UNION_CLASSES`(buriedAllClasses 포함)·전속 종족 4종(BURIED_RACES에 union 필드 → 위저드 게이트). 원전 오더(일일 공동 퀘스트)는 솔로에 무의미해 폐기) → 1.142 📜계약 2차(28→58종: 일반 10 + 보급 8 + **조직 상급 12**(union/unionLv 필드 — rollBuriedContract 2번째 인자 unionRep로 게이트, 미도달은 풀 제외). buriedContractCap에 조직 평판 Lv4/Lv8당 +4 — 전종 58 = 4정복+200층+전조직 만렙) → 1.143 🕳500층 엔드콘텐츠(PM 결정 「전부+인장 도입」 — **대역 5개** BURIED_ZONES(100층 단위 테마·보상 +10~50%, buriedZoneAt) + **수문장 5종 고정 배치**(로테이션 폐지, 400 태초의 포식자·500 묘주 신규, buriedBossKeyAt clamp — 600+는 묘주 반복) + **🗝수문장의 인장** BURIED_SIGILS(관문 첫 격파당 계정 영구 +8%/-4%, meta.buried.gateSigils + char.sigils — 생성 시 굽고 런 중 획득도 즉시 반영, 전투 envDmgPct/envTakenPct 소비) + **👑묘주 첫 격파**(apexClears + 「묘주의 관」 tomb1 — dungeon:'apex'로 풀 격리, rollBuriedUniqueItem forceId 확정 지급). 시뮬: 500층 격차 ×5.0 → 인장 5개 실효 ×2.86) → 1.144 🕯괴이 사역(괴력난신 원혼 설정, PM 5차 승인 — 상세 설계는 docs/pet-system-design.md. **BURIED_GHOSTS 26종**(기존 적 일러 재사용, 등급 5: 적/녹/청/남/자 — 자령 = 수문장·묘주) + **제령부**(rollBuriedTalisman: 보스 10% 고정·버프 무관·등급 60/25/9/5/1·상한 초과 먼지·**char.talismans 런 한정**) + **제령 행동**(전투 kind 'tame': HP≤25% + 부적, buriedTameChance = 기본치+(25−HP%)×0.5. 실패 = 소진+턴+[격노]3+전투 잠금) + **동행 1체**(meta.buried.ghosts/companion → char.ghost 생성 시 굽기. 패시브는 aggregateBuriedContracts에 병합 — 소비 지점 공짜, 단 takenPct만 전투 envTakenPct에 cf 라인 추가) + **턴 종료 자동 액티브**(적 행동보다 우선, ghostCd state) + **한계돌파**(동행 동일 개체 재제령, buriedGhostKit: %형 ×(1+0.2b)·스택 +1/돌파·수치형 +5) + 로비 「사역각」. ⚠️ 밸런스는 BURIED_GHOST_RANKS 한 곳)) / 이전 **1.90.0** (1.79~1.81 레이드 백그라운드·자동 사냥 확장 → 1.82 각성 스킬 15종 → 1.83~1.84 자동 전적·업적 롤백 픽스 → 1.85 도박장 → 1.86~1.87 레이드 난이도·초월 → 1.88 Wake Lock → 1.89 마스터즈+칭호 → **1.90.0 업적 전면 개편**: 죽은 업적 11개 부활·리뉴얼 + 신규 콘텐츠 업적 20개) |
