...
├── data/buried.js                # ★ 무덤의 유산 — BB 모티브 모드의 데이터+순수 로직 전부 (1.103.0)
├── utils/
...
| 현재 게임 버전 | **1.103.0** (1.103.0 ⚰ 무덤의 유산 신규 모드 — BuriedBornes 모티브 별도 모드) / 이전 **1.90.0** (1.79~1.81 레이드 백그라운드·자동 사냥 확장 → 1.82 각성 스킬 15종 → 1.83~1.84 자동 전적·업적 롤백 픽스 → 1.85 도박장 → 1.86~1.87 레이드 난이도·초월 → 1.88 Wake Lock → 1.89 마스터즈+칭호 → **1.90.0 업적 전면 개편**: 죽은 업적 11개 부활·리뉴얼 + 신규 콘텐츠 업적 20개) |
...
| **⚰ 무덤의 유산** (1.103.0) | `data/buried.js`(단일 데이터+순수함수 파일) + `components/buried/`(Screen·Dungeon·Battle·Manage·Common) + `meta.buried` + `FEATURE_FLAGS.buried` | **BuriedBornes(Nussy) 모티브 별도 모드**. PM 결정: "캐릭터만 살리고" — 본편에서 계승하는 것은 5직업의 이름·일러·컬러뿐. 패시브·유물·각인·노드맵·AP 일절 미사용. 핵심 6축: ①**장비=스킬**(6슬롯 각각에 스킬 1개 내장, 장비를 바꿔야 스킬이 바뀜) ②**SP**(턴당 회복, 기본 공격이 SP를 되돌림) ③**상태이상 13종**(스택형, 중독은 미감소·저주는 회복 완전 차단) ④**층 진행형 던전**(층당 방 2~3개 중 택1, 5층·10층 보스) ⑤**유산 계승형 사망**(캐릭터 소멸, 장착 장비 1~3개+골드 30% 계승) ⑥**레벨업 스탯 배분**(레벨당 3p). ⚠️ **밸런스 조정은 `data/buried.js` 한 곳** — `BURIED_TUNING`(전역 배율) / `buriedDerived`(파생 스탯·SP 압박) / 적 hp·atk. UI는 순수 함수를 호출만 하므로 node 시뮬로 검증 가능 |
