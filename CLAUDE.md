...
### 0.6. 이번 세션에서 학습한 PM 스타일 추가 패턴 (1.21.0 시점)
...
| 컨셉 | 일러 상태 | 비고 |
|---|---|---|
| frost (서리·동토) | ✅ 완료 (1.21.0) | combat 16 + intro 4 = 20장. 4보스 차별화 적용 (인간 전사/거인/사룡/무형 망령) |
| forest (부패·숲) | ❌ 프롬프트 미작성 | **다음 작업 1순위** |
| sanctum (신전·봉인) | ❌ 프롬프트 미작성 | |
| rift (마계·균열) | ❌ 프롬프트 미작성 | |
| dawn (천상·여명) | ❌ 프롬프트 미작성 | |
...
1. **챔피언십 forest 컨셉 프롬프트 20장 작성** — frost 룰 동일 적용:
   - 4난이도 환경 명확히 분리 (입문 / 중급 / 상급 / 지옥)
   - 4보스가 4가지 다른 존재 형태 (frost: 인간 전사 / 거인 / 사룡 / 무형 망령 → forest는 다른 4종)
   - 파일: `docs/enemy-illustration-prompts-championship-forest.md`
   - 작업 흐름: 프롬프트 PR → PM 생성 → 변환·코드 PR (frost 사이클 그대로 반복)
2. **챔피언십 sanctum 컨셉 프롬프트 20장** — 신전·봉인 (신성·법진·시간)
3. **챔피언십 rift 컨셉 프롬프트 20장** — 마계·균열 (마족·핏빛·차원)
4. **챔피언십 dawn 컨셉 프롬프트 20장** — 천상·여명 (천사·빛·골든)
...
- 다음 작업 1순위는 **챔피언십 forest 컨셉 프롬프트 20장** (11.1절)
- frost 룰(4보스 차별화, 4단계 환경 분리)을 동일 적용
