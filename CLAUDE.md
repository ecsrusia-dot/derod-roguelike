...
| **🔒 직업별 자동 플랜 — PM 룰 동결** (1.91.2 확정) | CombatScreen 방랑검사 분기(심안 3AP 플랜 + 소울 3룰) + App 드라이버 `WANDERER_SKILL_PRIO`(9순위 + 상위 5 부재 시 보석 리롤) | **⚠️ PM 명시 지시 (1.91.2 세션): 방랑검사 자동 AI 최적화 중단 — PM이 설정한 룰을 임의로 변경·"개선" 금지.** 전적분석(autoRunLog→AutoStatsScreen)은 **데이터 축적·표시 전용** — AI 룰에 자동 반영하는 코드를 만들지 말 것 (옵션 B 추천 조합도 PM 승인 전 구현 금지). 타 직업(술법사·마족·정령사·사제) 전용 플랜은 **PM이 직접 플레이 후 룰을 지시하면** 방랑검사와 동일 구조(전용 분기 + PRIO 배열)로만 구현 |
