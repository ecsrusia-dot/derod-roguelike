...
| **황혼의 벨트 — 포션** (1.96.0) | `data/potions.js`(POTIONS 4종·BELT_BASE_SLOTS 2) + App `belt` state(런 한정, 스냅샷 포함) + CombatScreen `handleUsePotion/chooseAutoPotion` + ShopScreen 포션 진열 + META_UPGRADES `meta_beltSlot`(+1/단계, 최대 4칸) | 사용: AP 미소모·턴당 1회(`player.usedPotionThisTurn` — endTurn 리셋). 자동 룰(PM 승인): HP<50% 최소 충족 물약 / 에테르 0 → 에테르 물약. 회복 보정(heal%) 미적용 — 표기값 그대로. 포션 추가는 POTIONS 항목 1줄 |
